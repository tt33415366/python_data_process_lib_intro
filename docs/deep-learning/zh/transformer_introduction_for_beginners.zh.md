...
在开始计算之前，让我们先理解**目的**和**我们将要实现什么**。

**目标：** 将每个独立的词向量转换为"上下文感知"的表示。

**为什么需要这样做？**

在原始输入中，每个词是独立的：
- `"cat"` → `[1.0, 0.5]`（只知道"cat"本身）
- `"sat"` → `[0.5, 1.0]`（只知道"sat"本身）

但理解句子需要知道词之间的关系！通过注意力，我们让每个词"看到"所有其他词：
- `"cat"` 的新表示 = 原始 `"cat"` + 来自 `"sat"` 的信息 + 来自 `"mat"` 的信息
...
┌─────────────────────────────────────────────────────────────┐
│  注：真实 Transformer 中，Q、K、V 是通过不同的权重矩阵    │
│  变换得到的（W_Q、W_K、W_V）。为简化计算，本例假设这些      │
│  是单位矩阵，因此 Q=K=V。                                   │
│                                                             │
│  在实际模型中：                                            │
│  Q = X × W_Q（查询投影）                                   │
...
原始 "cat"：  [1.0, 0.5]
新 "cat"：    [0.782, 0.719] ← 包含 "sat" 和 "mat" 的信息！
```
...
原始 "sat"：  [0.5, 1.0]
新 "sat"：    [0.752, 0.748] ← 包含 "cat" 和 "mat" 的信息！
```
...
原始 "mat"：  [0.8, 0.7]
新 "mat"：    [0.769, 0.731] ← 包含 "cat" 和 "sat" 的信息！
```
...
```
对角线（约36%）：
  每个词都关注自己——这是合理的，因为每个词都携带
  核心信息（身份、含义）

非对角线（30-34%）：
  "cat" 关注 "mat"（34%）→ 名词-位置关系
  "cat" 关注 "sat"（30%）→ 主语-动词关系
  "sat" 关注 "cat"（31%）→ 动词-主语关系
  "mat" 关注 "cat"（34%）→ 位置-实体关系
...
│  "cat" → [0.782, 0.719]                                    │
│                                                             │
│  变化：第二个维度从 0.5 → 0.719（增加了 44%！）             │
│                                                             │
│  为什么增加这么多？                                         │
│  "sat" 的第二个维度是 1.0（高值）                          │
│  "mat" 的第二个维度是 0.7（中高值）                        │
│  通过注意力，这些值"流"到了 "cat" 的表示中！              │
└─────────────────────────────────────────────────────────────┘
```
...
独立理解                     全局理解
    ┊                              ┊
"cat" 只知道自己           "cat" 知道整个句子
"sat" 只知道自己           "sat" 知道整个句子
"mat" 只知道自己            "mat" 知道整个句子
```
...
在深层 Transformer（如 GPT 的 96 层）中，经过多层这样的处理，
每个词的表示包含了**全局上下文**——这就是"理解"的数学基础！

**连接回第 3 节的问题：**

还记得我们问过"为什么需要 Transformer"吗？现在你有了答案：

```
问题："The trophy didn't fit in the suitcase because it was too large"
       中的 "it" 指的是什么？

答案：通过多层注意力，"it" 的表示会强烈关注 "trophy"
      （而不是 "suitcase"），因为模型学习到了语义关系！
```
...
└─────────────────────────────────────┘
      ...
      传递到下一层（前馈网络）
              ↓
      然后到下一个 Transformer 层
```

**关键洞察：**

1. **每个新表示都包含来自所有其他词的信息**
2. **这些新向量成为下一层的输入**
3. **在多层 Transformer 中，这个过程重复多次**
4. **每一层学习越来越抽象的关系**

**连接到架构：**

在完整的 Transformer 中（如第 5 节所述）：
- 编码器层 N 的输出 → 解码器的交叉注意力
- 解码器使用这些来生成翻译/摘要等
- 这就是"理解"如何在网络中流动！
...
注意力可视化会显示：
- "it" 强烈关注 "trophy"（因为它是大的）
- "fit" 关注 "trophy" 和 "suitcase"（动作关系）
```
...
**实际应用：**
- 这就是 ChatGPT、BERT 等模型"理解"文本的方式
- 每层都做这个，每次学习更深层的关系
- 多头注意力并行执行此操作多次（第 4.4 节）

**下一步：**
现在你理解了单个注意力层的计算。完整的 Transformer：
1. 使用多个头（并行计算不同的注意力模式）
2. 多层堆叠（学习越来越抽象的关系）
3. 前馈网络（处理每个上下文感知表示）
4. 编码器-解码器（用于翻译等生成任务）
...
❌ 错误：自注意力只能看到附近的词
✓ 正确：自注意力是全局的（每个标记能看到每个标记）
  它学习哪些连接是重要的，而不是受距离限制
...
示例：在 "The cat sat" 中：
- 头 1："cat" 高度关注 "sat"（主语-动词关系）
- 头 2："cat" 高度关注 "The"（冠词一致性）
- 两者都是"正确"的注意力，只是学习不同的模式
