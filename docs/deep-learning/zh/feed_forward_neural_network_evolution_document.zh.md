...
**心智模型 / 类比：**
将前向传播想象成工厂的装配线。原材料（输入数据）从一端进入，每个工作站（神经元）根据其特定指令（权重和激活函数）处理材料，将半成品传递给下一个工作站。最终产品（预测）在生产线的末端出现。

前向传播是输入数据通过网络产生输出的过程。对于每个神经元，这涉及：

1. **加权和：** `z = sum(w_i * x_i) + b`，其中 `w_i` 是权重，`x_i` 是输入，`b` 是偏置
2. **激活：** `a = f(z)`，其中 `f` 是激活函数（ReLU、sigmoid等）
3. **传播：** 输出 `a` 成为下一层的输入
...
graph TD
    ...
    subgraph Hidden_Layer["第一个隐藏层"]
        H1["神经元 h₁:<br/>z₁ = w₁₁x₁ + w₁₂x₂ + w₁₃x₃ + b₁<br/>a₁ = ReLU(z₁)"]
        H2["神经元 h₂:<br/>z₂ = w₂₁x₁ + w₂₂x₂ + w₂₃x₃ + b₂<br/>a₂ = ReLU(z₂)"]
    end

    subgraph Output_Layer["输出层"]
        O1["神经元 y₁:<br/>z₃ = w₃₁a₁ + w₃₂a₂ + b₃<br/>output = σ(z₃)"]
    end
...
**心智模型 / 类比：**
想象你在练习射箭但没有射中靶心。反向传播就像分析*为什么*你没射中："我瞄准太高了，拉弓太紧"。然后你在心理上调整下一次射击的技巧。在神经网络中，这种"调整"以数学方式发生，从输出误差向后传播，以确定每个权重和偏置对错误的贡献程度。

反向传播是使神经网络能够学习的算法。它计算损失函数相对于每个权重的梯度，指示每个参数应该调整多少以减少误差。

**反向传播过程：**

1. **前向传播：** 计算预测并计算损失（与真实值的差异）
2. **反向传播：** 使用链式法则计算梯度
   - 输出层梯度：`∂L/∂z_output`
   - 隐藏层梯度：通过每层向后传播误差
   - 权重梯度：`∂L/∂w = (∂L/∂z) * (∂z/∂w)`
3. **权重更新：** 使用优化器调整权重：`w_new = w_old - learning_rate * gradient`

**Mermaid 图：反向传播流程**

```mermaid
graph TB
    subgraph Forward["前向传播（计算输出）"]
        direction TB
        F1["输入: x"]
        F2["隐藏层: h = ReULU(Wx + b)"]
        F3["输出: ŷ = σ(W'h + b')"]
        F4["损失: L = loss(ŷ, y_true)"]
    end

    subgraph Backward["反向传播（计算梯度）"]
        direction TB
        B1["∂L/∂ŷ = 损失的导数"]
        B2["∂L/∂W' = ∂L/∂ŷ × ∂ŷ/∂W'<br/>(链式法则)"]
        B3["∂L/∂h = 通过激活函数<br/>反向传播误差"]
        B4["∂L/∂W = ∂L/∂h × ∂h/∂W"]
    end

    subgraph Update["更新权重"]
        direction TB
        U1["W ← W - η × ∂L/∂W<br/>(η = 学习率)"]
    end
...
**心智模型 / 类比：**
想象你有无限数量的柔性积木（如乐高积木或柔性管）。即使只有一层这样的积木，只要你拥有足够的数量并且能够正确排列它们，你就可以构建或近似几乎任何可以想象到的形状。万能近似定理说明这对神经网络来说是成立的：具有足够神经元的单个隐藏层可以近似任何连续函数。

**万能近似定理**（由Cybenko于1989年，Hornik于1991年证明）是神经网络理论的基础结果。它指出：

> **具有单个隐藏层且包含有限数量神经元的前馈神经网络可以以任意精度近似紧致子集 ℝⁿ 上的任何连续函数，前提是激活函数是非恒定、有界且连续的（例如sigmoid）。**

**关键含义：**

1. **理论能力：** FFNN是*万能函数逼近器*——它们可以表示从输入到输出的任何连续映射，只要有足够的隐藏神经元。

2. **实际限制：** 虽然定理保证近似是*可能的*，但它没有说明：
   - 需要多少神经元
   - 如何找到正确的权重
   - 训练需要多长时间

   在实践中，深度网络（多个隐藏层）通常比浅层宽网络更有效。

3. **深度效率：** 虽然单个隐藏层在理论上是充分的，但深度网络可以更有效地表示某些函数（需要更少的总神经元）。

**为什么深度网络仍然重要：**

该定理可能表明浅网络就足够了，但深度网络提供了优势：
- **分层特征学习：** 每层可以构建 increasingly 抽象的特征
- **参数效率：** 深度网络可以用指数级更少的参数表示某些函数
- **泛化能力：** 深度网络通常从有限数据中更好地泛化
- **优化景观：** 深度网络可以具有使训练更稳定的优化特性

**示例：** 考虑近似复杂的决策边界。浅层网络可能在一个隐藏层中需要数千个神经元，而跨多个层的每层只有几个神经元的深度网络可以更高效地实现相同的近似。
...
**心智模型 / 类比：**
想象开始一场比赛。如果你从队伍的最后开始，你将很难赶上。如果你远远领先于其他人，你可能会变得自满。权重初始化就是为训练选择一个好的起跑位置——不能太小，不能太大，恰到好处。

权重初始化是在训练开始之前设置神经网络权重初始值的过程。适当的初始化对于有效训练至关重要——糟糕的初始化可能导致梯度消失/爆炸、收敛缓慢或完全无法学习。

**初始化为何重要：**

当权重初始化不当时：
- **太小：** 梯度在向后传播时消失；早期层的神经元学习非常缓慢
- **太大：** 梯度爆炸；激活变得饱和；训练变得不稳定
- **恰到好处：** 梯度平滑流动；所有层以相似的速率学习；训练高效收敛

**常见初始化策略：**

##### 3.5.1. 随机初始化（历史方法）

- **方法：** 从具有小随机值的均匀分布或正态分布中采样
- **问题：** 不考虑网络深度或激活函数属性
- **状态：** 基本被更好的方法取代

##### 3.5.2. Xavier/Glorot 初始化（适用于 Sigmoid/Tanh）

- **公式：** `W ~ U(-√(6/(n_in + n_out)), √(6/(n_in + n_out)))` 或 `N(0, √(2/(n_in + n_out)))`
- **设计用于：** Sigmoid 和 Tanh 激活函数
- **直觉：** 根据输入和输出连接的数量缩放权重，以维持跨层的激活和梯度的方差
- **用例：** 使用 sigmoid 或 tanh 激活函数的网络

##### 3.5.3. He 初始化（适用于 ReLU）

- **公式：** `W ~ N(0, √(2/n_in))`
- **设计用于：** ReLU 及其变体（Leaky ReLU、Parametric ReLU）
- **直觉：** ReLU 将负值归零，因此方差减半；He 初始化对此进行补偿
- **用例：** 大多数使用 ReLU 族激活函数的现代深度网络（许多框架中的默认选择）

**快速参考：**

| 初始化方法 | 最佳激活函数 | 框架关键字 |
|:---|:---|:---|
| Xavier/Glorot 均匀 | Sigmoid、Tanh、Softmax | `glorot_uniform` |
| Xavier/Glorot 正态 | Sigmoid、Tanh、Softmax | `glorot_normal` |
| He 正态 | ReLU、Leaky ReLU | `he_normal` |
| He 均匀 | ReLU、Leaky ReLU | `he_uniform` |
...
# 使用 Xavier 初始化（用于 sigmoid/tanh）
layer = layers.Dense(64, activation='sigmoid',
                    kernel_initializer='glorot_uniform')
```

**实用指导：**
- 对于 ReLU 网络（最常见）：使用 **He 初始化**
- 对于 sigmoid/tanh 网络：使用 **Xavier/Glorot 初始化**
- 框架默认值通常是明智的选择
- 如果训练无法收敛，请尝试不同的初始化策略
